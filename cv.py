...
def grafico_pizza_focado_desfocado():
    fatias = [23,5]
    status = ["Focado(a)","Desfocado(a)"]
    cores = ["green", "red"]
    plt.pie(fatias,labels=status, colors=cores, startangle=90, shadow=True)
    plt.title("Tempo total em segundos passado focado e desfocado")
    plt.show()
...
def enviar_email_via_smtl():
    servidor_email = smtp.SMTP('smtp.gmail.com', 587)
    servidor_email.starttls()
    servidor_email.login(os.getenv("EMAIL"), os.getenv("SENHA_EMAIL"))
    remetente = os.getenv("EMAIL")
    destinatario = "barbarangelinn@gmail.com"
    conteudo = "Subject: KAIROS - HISTORICO DE OBSERVACOES\n\nOla tudo bem :)"
